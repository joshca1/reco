...
class sigesp_rcm_c_sno
{
	...
	{	
		...
		if($lb_valido)
		{	
			$lb_valido=$this->uf_convertir_snord();
		}
		if($lb_valido)
		{	
			$lb_valido=$this->uf_convertir_snosueldoshistoricos();
		}
		if($lb_valido)
		{	
	...
	function uf_convertir_snocestaticket()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","moncestic");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_cestaticket",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	//-----------------------------------------------------------------------------------------------------------------------------
	function uf_convertir_snosueldoshistoricos()
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: snosueldoshistoricos
		//		   Access: private
		//	      Returns: lb_valido True si se ejecuto el insert ó False si hubo error en el insert
		//	  Description: Funcion que selecciona los campos de moneda de la tabla sno_sueldoshistoricos e inserta el valor convertido
		//	   Creado Por: santi
		// Fecha Creación: 06/08/2007 								Fecha Última Modificación : 
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT codemp, codper, fecsue, ".
				" suebas, sueint, sueprodia, bonvac, bonfinanio, otrasig, confij, convar".
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_sno MÉTODO->SELECT->snosueldoshistoricos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_fecsue= $row["fecsue"];
				$li_suebas= $row["suebas"];
				$li_sueint= $row["sueint"];
				$li_sueprodia= $row["sueprodia"];
				$li_bonvac= $row["bonvac"];
				$li_bonfinanio= $row["bonfinanio"];
				$li_otrasig= $row["otrasig"];
				$li_confij= $row["confij"];
				$li_convar= $row["convar"];
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","suebas");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_suebas);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueint");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueint);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueprodia");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueprodia);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonvac);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonfinanio");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonfinanio);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","otrasig");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_otrasig);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","confij");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_confij);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","convar");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_convar);
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("filtro","fecsue");
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_fecsue);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_sueldoshistoricos",$la_seguridad);
			}
		}		
		return $lb_valido;
	}// end function snosueldoshistoricos
	//-----------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------
		function uf_convertir_snord()
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_convertir_snord
		//		   Access: private
		//	      Returns: lb_valido True si se ejecuto el insert ó False si hubo error en el insert
		//	  Description: Funcion que selecciona los campos de moneda de la tabla sno_rd e inserta el valor convertido
		//	   Creado Por: santi consultores santi.com.ve
		// Fecha Creación: 27/05/2018 								Fecha Última Modificación : 
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT codemp, codnom, codperi, codper, sc_cuenta, debhab, monpagper".
				"  FROM sno_rd".
				" WHERE codemp='".$this->ls_codemp."'";
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_sno MÉTODO->SELECT->uf_convertir_snord ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monpagper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monpagper);
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_rd",$la_seguridad);
			}
		}		
		return $lb_valido;
	}// end function uf_convertir_snord
	//-----------------------------------------------------------------------------------------------------------------------------
	...
	function uf_convertir_snoclasificaciondocente()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","suesupcladoc");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_suesupcladoc);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","suedircladoc");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_suedircladoc);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","suedoccladoc");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_clasificaciondocente",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoconcepto()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acumaxcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acumaxcon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valmincon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valmincon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valmaxcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valmaxcon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valminpatcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valminpatcon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valmaxpatcon");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_concepto",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoconceptopersonal()
	{
		...
		$ls_sql="SELECT codemp, codnom, codper, codconc, valcon, acuemp, acuiniemp, acupat, acuinipat ".
				...
				" WHERE codemp='".$this->ls_codemp."' and (valcon <0 or valcon >0) or (acuemp <0 or acuemp >0) ".
				"or (acuiniemp <0 or acuiniemp >0) or (acupat <0 or acupat >0) or (acuinipat <0 or acuinipat >0) " ;
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valcon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acuemp");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acuemp);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acuiniemp");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acuiniemp);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acupat");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acupat);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acuinipat");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_conceptopersonal",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoconceptovacacion()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acumaxsalvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acumaxsalvac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","minsalvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_minsalvac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","maxsalvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_maxsalvac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","minpatsalvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_minpatsalvac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","maxpatsalvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_maxpatsalvac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","acumaxreivac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_acumaxreivac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","minreivac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_minreivac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","maxreivac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_maxreivac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","minpatreivac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_minpatreivac);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","maxpatreivac");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_conceptovacacion",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoconstante()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_valcon= $row["valcon"];
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","equcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_equcon);
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","topcon");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_constante",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoconstantepersonal()
	{
		...
		$ls_sql="SELECT codemp, codnom, codper, codcons, moncon, montopcon".
				...
				" WHERE codemp='".$this->ls_codemp."' and (moncon !=0 or montopcon!=0)";
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_montopcon= $row["montopcon"];
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","moncon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_moncon);
				$this->io_rcbsf->io_ds_datos->insertRow("campo","montopcon");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_montopcon);
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_constantepersonal",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snodtscg()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_dt_scg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snodtspg()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_dt_spg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snofideiperiodo()
	{
		...
		$ls_sql="SELECT codemp, codnom, codper, anocurper, mescurper, bonvacper, bonfinper, sueintper, apoper, bonextper, ".
				"  bonvacadiper, bonfinadiper, sueintadiper, apopreper ".
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_bonvacadiper= $row["bonvacadiper"];
				$li_bonfinadiper= $row["bonfinadiper"];
				$li_sueintadiper= $row["sueintadiper"];
				$li_apopreper= $row["apopreper"];

				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonvacadiper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonvacadiper);
				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonfinadiper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonfinadiper);
				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueintadiper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueintadiper);
				$this->io_rcbsf->io_ds_datos->insertRow("campo","apopreper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_apopreper);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonvacper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonvacper);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonfinper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_bonfinper);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueintper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueintper);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","apoper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_apoper);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","bonextper");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_fideiperiodo",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snogrado()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monsalgra");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monsalgra);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","moncomgra");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_grado",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoperiodo()
	{
		...
		{  
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","totper");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_periodo",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snopersonal()
	{
		...
		{  
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monpagvivper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monpagvivper);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","ingbrumen");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_personal",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snopersonalnomina()
	{
		...
		{  
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueper);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueintper");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueintper);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueproper");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_personalnomina",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprenomina()
	{
		...
		{  
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valprenom");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valprenom);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","valhis");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_prenomina",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprestamos()
	{
		...
		{ 
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monpre");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monpre);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monamopre");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_prestamos",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprestamosamortizado()
	{
		...
		{ 
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monamo");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_prestamosamortizado",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprestamosperiodo()
	{
		...
		{ 
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","moncuo");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_prestamosperiodo",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprimaconcepto()
	{
		...
		{ 
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valpri");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_primaconcepto",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snoprimagrado()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monpri");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_primagrado",$la_seguridad);
			}
		}		
		return $lb_valido;
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_mondic= $row["mondic"];
   				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","totasi");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_totasi);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monene");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monene);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monfeb");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monfeb);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monmar");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monmar);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monabr");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monabr);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monmay");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monmay);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monjun");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monjun);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monjul");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monjul);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monago");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monago);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monsep");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monsep);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monoct");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monoct);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monnov");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monnov);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","mondic");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_programacionreporte",$la_seguridad);
			}
		}		
		return $lb_valido;
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_monnetres= $row["monnetres"];
   				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","asires");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_asires);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","dedres");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_dedres);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","apoempres");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_apoempres);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","apopatres");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_apopatres);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","priquires");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_priquires);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","segquires");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_segquires);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monnetres");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_resumen",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_snosalida()
	{
		...
		$ls_sql="SELECT codemp, codnom, codperi, codper, codconc, tipsal, valsal, monacusal, salsal ".
				"  FROM sno_salida ".
				" WHERE codemp='".$this->ls_codemp."' and (valsal!=0 or monacusal!=0 or salsal!=0) ";
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_salsal= $row["salsal"];
   				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","valsal");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_valsal);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monacusal");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monacusal);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","salsal");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_salida",$la_seguridad);
			}
		}		
		return $lb_valido;
		...
		{ //
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_ultsuetraant= $row["ultsuetraant"];
   				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","ultsuetraant");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_trabajoanterior",$la_seguridad);
			}
		}		
		return $lb_valido;
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_monto_5= $row["monto_5"];
   				// Campos a Convertir
				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueintbonvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueintbonvac);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","sueintvac");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_sueintvac);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monto_1");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monto_1);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monto_2");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monto_2);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monto_3");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monto_3);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monto_4");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monto_4);

				$this->io_rcbsf->io_ds_datos->insertRow("campo","monto_5");
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("sno_vacacpersonal",$la_seguridad);
			}
		}		
		return $lb_valido;
