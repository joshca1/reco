...
/*		$ld_fecha=date("Y_m_d_H_i");
	...
	{	
		...
		// if($lb_valido) no existe en enterprise
		// {	
		// 	$lb_valido=$this->uf_convertir_cxpanticipos();
		// }
		// if($lb_valido) no existe en enterprise
		// {	
		// 	$lb_valido=$this->uf_convertir_cxpdtanticipos();
		// }
		if($lb_valido)
		{	
	...
	function uf_convertir_cxpdcscg()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_dc_scg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxpdcspg()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_dc_spg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxpdtsolicitudes()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_codpro);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_dt_solicitudes",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxprd()
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT codemp, numrecdoc, codtipdoc, ced_bene, cod_pro,  montotdoc, mondeddoc, moncardoc, montot".
				"  FROM cxp_rd".
				" WHERE codemp='".$this->ls_codemp."'";
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxprd ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_montot= $row["montot"];//agregado nuevo aparece en la tabla enterprise santi

				$this->io_rcbsf->io_ds_datos->insertRow("campo","montotdoc");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_montotdoc);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","mondeddoc");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_mondeddoc);
	
				$this->io_rcbsf->io_ds_datos->insertRow("campo","moncardoc");
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_codpro);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_rd",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxprdcargos()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxprdcargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monobjret");
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_rd_cargos",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxprddeducciones()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxprddeduccioness ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_numdoccom);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_rd_deducciones",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxprdscg()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxprdscg ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_sccuenta);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_rd_scg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxprdspg()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxprdspg ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_spgcuenta);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_rd_spg",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxpsolbanco()
	{
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_sol_banco",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxpsoldc()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxpsoldc ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_numdc);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_sol_dc",$la_seguridad);
			}
		}		
		return $lb_valido;
	...
	function uf_convertir_cxpsolicitudes()
	{
		...
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->uf_convertir_cxpsolicitudes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$this->io_rcbsf->io_ds_datos->insertRow("campo","monsol");
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_numsol);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_solicitudes",$la_seguridad);
			}
		}		
		return $lb_valido;
	}// end function uf_convertir_cxpsolicitudes
	//-----------------------------------------------------------------------------------------------------------------------------
	function uf_convertir_cxpanticipos()
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_convertir_cxpanticipos
		//		   Access: private
		//	      Returns: lb_valido True si se ejecuto el insert ó False si hubo error en el insert
		//	  Description: Funcion que selecciona los campos de moneda de la tabla cxp_anticipos e inserta el valor convertido
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creación: 20/07/2007 								Fecha Última Modificación : 
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT numrecdoc, nroanticipo, montoant".
				"  FROM cxp_anticipos";
		$rs_data=$this->io_sql_origen->select($ls_sql);
		if($rs_data===false)
		{ 
			$this->io_mensajes->message("CLASE->sigesp_rcm_c_cxp MÉTODO->SELECT->anticipos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($rs_data))&&($lb_valido))
			{
				$ls_codemp= $row["nroanticipo"]; 
				$ls_numrecdoc= $row["numrecdoc"];
				$li_monsol= $row["montoant"];
				
				$this->io_rcbsf->io_ds_datos->insertRow("campo","montoant");
				$this->io_rcbsf->io_ds_datos->insertRow("monto",$li_monsol);

				$this->io_rcbsf->io_ds_filtro->insertRow("filtro","nroanticipo");
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_codemp);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_numrecdoc);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_anticipos",$la_seguridad);
			}
		}		
		return $lb_valido;
	}// end function anticipos
	//-----------------------------------------------------------------------------------------------------------------------------
	function uf_convertir_cxpdtanticipos()
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_convertir_cxpdtanticipos
		//		   Access: private
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT nroanticipo, nrodesc, montodesc".
		...
		else
		{
			...
			{
				$ls_codemp= $row["nroanticipo"]; 
				$ls_numrecdoc= $row["nrodesc"];
				$li_monsol= $row["montodesc"];
				...
				$this->io_rcbsf->io_ds_filtro->insertRow("valor",$ls_numrecdoc);
				$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("cxp_dt_anticipos",$la_seguridad);
			}
		}		
		return $lb_valido;
	}// end function uf_convertir_cxpsolicitudes
